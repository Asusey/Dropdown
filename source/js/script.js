...
window.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('load', () => {
    ...
    // открытие дропдауна
    const initDropdownOpener = () => {
      const dropdownButton = document.querySelector('.form__button');
      ...
      // функция показа и скрытия дропдауна
      const showDropdown = () => {
      ...
      const closeDropdown = () => {
        ...
        // Функция для скрытия дропдауна при клике на "Escape"
    ...
    const buttonCounter = document.querySelector('.form__button-counter');
    const optionBlock = document.querySelector('.check-option-block');
    ...
    const isChecked = () => {
      checkboxes.forEach((input) => {
        input.addEventListener('click', () => {
          if (input.checked) {
            count++;
            buttonCounter.classList.add('is-active');
            optionBlock.classList.add('is-active');
          } else {
            count--;
          }
          ...
          if (count === 0) {
            buttonCounter.classList.remove('is-active');
            optionBlock.classList.remove('is-active');
          }
        });
      });
    }
    ...
    // добавление интерактивного списка
    (function( $ ){
      ...
      function addCheckOption() {
        document.getElementById('check-option-block').innerHTML = "";

        $('.option-item input').each(function () {

          var optionTitle = $(this).data('name');
          var optionList = document.querySelector('.check-option-block');
          ...
          optionButton.className = 'button-item-close';
    ...
    // скрытие элементов инерактивного списка
    // const interactivBox = document.querySelector('.check-option-block');
    // let index = -1;

    // interactivBox.addEventListener('click', () => {
    //   const interactivItems = interactivBox.querySelectorAll('.button-item-close');

    //   for (let i = 0; i < interactivItems.length; i++) {
    //     interactivItems[i].addEventListener('click', () => {
    //       if (interactivItems[i] >= 0) {
    //         index = i;
    //         interactivItems[i].parentNode.remove();
    //       }

    //       if (index === -1) {
    //         interactivBox.classList.remove('is-active');
    //       }
    //     });
    //   }
